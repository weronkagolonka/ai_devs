...
export class ArticleService {
    ...
    private articleMarkdownDirectory = "./S02E05/markdown";
    private acrticleMarkdownFilePath = `${this.articleMarkdownDirectory}/article.md`;

    private answersDirectory = "./S02E05/answers";
    ...
    async getArticleAsMarkdown() {
        ...
        if (markdownFile) {
            ...
            for (const heading of $("h2")) {
                if (
                    !$(heading).text().includes("Abstrakt") &&
                    !$(heading).text().includes("Źródła")
                ) {
                    ...
                    for (const element of $(heading).nextUntil("h2")) {
                        if (
                            ...
                            const res = await this.openAiService.describeImage(
                                fullImageUrl,
                                caption
                            );
                            markdown += `<img>\n${res}\n</img>\n<caption>\n${caption}\n</caption>\n\n`;
                            ...
                            const audioUrl = $(element)
                                .find("source")
                                .attr("src");
                            const fullAudioUrl = `${this.articleBaseDomain}/${audioUrl}`;

                            const res =
                                await this.openAiService.transcribeAudio(
                                    fullAudioUrl
                                );
                            markdown += `<audio>\n${res}\n</audio>\n\n`;
    ...
    async answerQuestions(
        ...
        if (answersFile) {
            ...
            for (const q of questions) {
                const answer = cachedAnswers.find(
                    (a) => a.question.id === q.id
                );
                if (!answer) {
                    console.log("Cache miss");
                    const newAnswer = await this.openAiService.answerQuestion(
                        q.question,
                        article
                    );
                    console.log(
                        `answered question: ${q.id}\nanswer: ${newAnswer?.answer}\nthinking: ${newAnswer?.thinking}\n\n`
                    );
